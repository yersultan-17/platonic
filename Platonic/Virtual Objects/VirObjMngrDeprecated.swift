...
class VirtualObjectManager {
   ...
   /* weak var delegate: VirtualObjectManagerDelegate?
    ...
    static let availableObjects = [VirtualObjectDefinition(type: .cube, image: #imageLiteral(resourceName: "cubeIcon"), custom: false),
                                   VirtualObjectDefinition(type: .rectPrism, image: #imageLiteral(resourceName: "recPrismIcon"), custom: false),
                                   VirtualObjectDefinition(type: .trianglePrism, image: #imageLiteral(resourceName: "trianglePrismIcon"), custom: true), // custom
                                   VirtualObjectDefinition(type: .sphere, image: #imageLiteral(resourceName: "sphereIcon"), custom: false),
                                   VirtualObjectDefinition(type: .cone, image: #imageLiteral(resourceName: "coneIcon"), custom: false),
                                   VirtualObjectDefinition(type: .cuttedCone, image: #imageLiteral(resourceName: "cuttedConeIcon"), custom: false),
                                   VirtualObjectDefinition(type: .cylinder, image: #imageLiteral(resourceName: "cylinderIcon"), custom: false),
                                   VirtualObjectDefinition(type: .rectPyramid, image: #imageLiteral(resourceName: "rectPyramidIcon"), custom: false),
                                   VirtualObjectDefinition(type: .trianglePyramid, image: #imageLiteral(resourceName: "trianglePyramidIcon"), custom: true), // custom
                                   VirtualObjectDefinition(type: .plane, image: #imageLiteral(resourceName: "planeIcon"), custom: false),
                                   VirtualObjectDefinition(type: .line, image: #imageLiteral(resourceName: "lineIcon"), custom: true)] // custom
 ...
 /*   func getDimensions(object: VirtualObject) -> [String: CGFloat] {
    ...
    private func setNewVirtualObjectPosition(_ object: VirtualObject, to pos: float3, cameraTransform: matrix_float4x4) {
        ...
        object.simdPosition = cameraToPosition + cameraWorldPos
        if object.definition.type == .sphere {
            object.simdPosition.y += (Float(object.dimensions["Radius"]!))
        } else if (!object.definition.custom && object.definition.type != .rectPyramid) {
    ...
    private func updateVirtualObjectPosition(_ object: VirtualObject, to pos: float3, filterPosition: Bool, cameraTransform: matrix_float4x4) {
        ...
        if filterPosition, let averageDistance = object.recentVirtualObjectDistances.average {
            let averageDistancePos = cameraWorldPos + simd_normalize(cameraToPosition) * averageDistance
            object.simdPosition = averageDistancePos
        } else {
            object.simdPosition = cameraToPosition + cameraWorldPos
        }
        if object.definition.type == .sphere {
    ...
    func checkIfObjectShouldMoveOntoPlane(anchor: ARPlaneAnchor, planeAnchorNode: SCNNode) {
        for object in virtualObjects {
            ...
            let objectPos = planeAnchorNode.convertPosition(object.position, to: object.parent)
